...
from pointcloud_helper import my_write_ply
...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('loss'):
            ...
            tf.print(reducing_list, output_stream='stderr')
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            #     sess_temp.run(tf.global_variables_initializer())
            #     sess_temp.run(dataset.train_init_op)
            #     raw_inputs = sess_temp.run(dataset.flat_inputs)
            # raw_xyz = raw_inputs[0]
            # labels = raw_inputs[21]
            # filenames = raw_inputs[-1]
            # for i, filename in enumerate(filenames):
            #     filename_str = filename.decode('utf-8')
            #     if filename_str not in files_cur_epoch:
            #         files_cur_epoch.append(filename_str)
            #         print("filename:", filename_str)
            #         save_folder = "temp_ply"
            #         name = "{}_epoch{}.ply".format(filename_str, self.training_epoch)
            #         my_write_ply(join(save_folder, name), [raw_xyz[i], labels[i]], ["x","y","z", "label"])
                    # my_write_ply(join(save_folder, name), [raw_xyz[i][0, :, :], labels[i][0, :]], ["x","y","z", "label"])
