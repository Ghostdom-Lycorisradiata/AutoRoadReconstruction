...
class DataProcessing:
    ...
    @staticmethod
    def data_aug(xyz, color, normal, curvatures, labels, idx, num_out):
        num_in = len(xyz)
        ...
        normal_dup = normal[dup, ...]
        normal_aug = np.concatenate([normal, normal_dup], 0)
        idx_dup = list(range(num_in)) + list(dup)
        idx_aug = idx[idx_dup]
        curvature_aug = curvatures[idx_dup]
        label_aug = labels[idx_dup]
        return xyz_aug, color_aug, normal_aug, curvature_aug, idx_aug, label_aug
    ...
    def get_class_weights(dataset_name):
        ...
        elif dataset_name is 'SensatUrban':
            # num_per_class = np.array([13245696, 8340565, 31469363, 3155108, 0, 65308,0,
            #                           11303519, 1587361, 2563329, 0, 0, 0],dtype=np.int32)
            num_per_class = np.array([13245696,  58484553],dtype=np.int32)
    ...
    def calculate_pc_normals(pc_xyz):
        ...
        pc_normals = pc_normals.astype(np.float32)
        return pc_normals

    @staticmethod
    def calculate_pc_features(pc_xyz):
        pc = open3d.PointCloud()
        pc.points = open3d.Vector3dVector(pc_xyz)
        # 估计法向量
        open3d.estimate_normals(pc)
        ...
        for i in range(len(pc_xyz)):
            radius = 0.5
            while True:
                [k, idx, _] = pcd_tree.search_radius_vector_3d(pc_xyz[i], radius)
                if k >= 7:  # 如果找到足够点，停止扩大半径
                    print("min_radius:", radius)
                    break
                else:
                    radius += 0.05  # 增加半径继续搜索

            # 邻域点法向量
            # normals_neighbors = pc_normals[idx]       
            # 计算法向量的平均和方差
            mean_normal = np.mean(normals_neighbors, axis=0)
            mean_normal /= np.linalg.norm(mean_normal)  # 单位化
            # normal_variance = np.std(normals_neighbors, axis=0)
            # 计算曲率
            curvature = np.linalg.norm(mean_normal - pc_normals[i])
            curvatures.append(curvature)
            # 计算边缘强度
            # edge_strength = np.linalg.norm(np.mean(normals_neighbors, axis=0) - pc_normals[i])
            # edge_strengths.append(edge_strength)
            # 计算局部密度
            # local_density = k / (np.pi * radius ** 2)  # 基于邻域点数计算密度
            # local_densities.append(local_density)
            # 计算点云稀疏度
            # sparsity = 1 / local_density if local_density > 0 else 0  # 稀疏度
            # sparsities.append(sparsity)
            # 法向量方差
            # normal_variances.append(np.linalg.norm(normal_variance))

        return (pc_normals,  # 返回法向量
                np.array(curvatures, dtype=np.float32))
                # np.array(edge_strengths, dtype=np.float32), 
                # np.array(local_densities, dtype=np.float32), 
                # np.array(sparsities, dtype=np.float32), 
                # np.array(normal_variances, dtype=np.float32))
