                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_val):
                            ...
                            write_ply(join(test_path, name), [xyz, preds], ['x', 'y', 'z', 'label'])
                            #write_ply(join(test_path, 'val_preds', name), [preds, labels], ['pred', 'label'])
                            csv_name = pointcloud_to_csv(join(test_path, name))
                            apls_result = apls('data/images', join(test_path, csv_name), join('data/sensat', csv_name))
                             # Ensure apls_result is a valid number (not NaN, None, or invalid type)
                            if isinstance(apls_result, (int, float)) and not np.isnan(apls_result):
                                apls_result_sum += apls_result
                                apls_result_count += 1
                                #print("current apls_result:", apls_result)
                            else:
                                print(f"Invalid apls_result at index {i_test}: {apls_result}")
                        ...
                        m_APLS = apls_result_sum / apls_result_count
                        print("average_apls_result", m_APLS)
