...
"""
# 使用命令
# python apls_args.py --test_method=gt_wkt_prop_wkt --output_name=gt_wkt_prop_wkt --gt_wkt_file=data/gt_wkt_prop_wkt/ground_truth_randomized/gt.csv --wkt_file=data/gt_wkt_prop_wkt/proposal/pro.csv --im_dir=data/images
    ...
    for i, (u, v, attr_dict) in enumerate(G_gt_almost.edges(data=True)):
        ...
        for sk in speed_keys:
            ...
            if type(attr_dict[sk]) == list:
                ...
                # if verbose:
                #     print("u, v, speed_key, attr_dict)[speed_key]:", u, v, sk, attr_dict[sk])
...
def make_graphs_yuge(G_gt, G_p,
    ...
    if verbose:
        ...
        # try:
        #     print("edge:", edge_tmp, "G_gt random edge props:", G_gt.edges[edge_tmp[0]][edge_tmp[1]])
        # except:
    ...
    if verbose:
        print(("Time to compute all source routes for", sample_size, "nodes:", time.time() - tt, "seconds"))
    ...
    if verbose:
        print(("Time to compute all source routes for", max_nodes, "nodes:", time.time() - tt, "seconds"))
...
def gather_files(test_method, truth_dir, prop_dir,
    ...
    if test_method == 'gt_pkl_prop_pkl':
        ...
        for i, f in enumerate(name_list):
            ...
            # print("gt random edge props for edge:", edge_tmp, " = ", G_gt_init.edges[edge_tmp[0], edge_tmp[1], 0])
            ...
            # print("prop random edge props for edge:", edge_tmp, " = ", G_p_init.edges[edge_tmp[0], edge_tmp[1], 0])
...
#            print("image_id", image_id)
            ...
            if verbose:
                ...
                edge_tmp = list(G_gt_init.edges())[-1]
                # G.edge[edge_tmp[0]][edge_tmp[1]])
                # print("random edge props for edge:", edge_tmp, " = ", G_gt_init.edges[edge_tmp[0], edge_tmp[1], 0])
            ...
            if verbose:
                ...
                edge_tmp = list(G_p_init.edges())[-1]
                # G.edge[edge_tmp[0]][edge_tmp[1]])
                # print("random edge props for edge:", edge_tmp, " = ", G_p_init.edges[edge_tmp[0], edge_tmp[1], 0])
...
def execute(output_name, gt_list, gp_list, root_list, im_loc_list=[],
    ...
    return np.mean(df['APLS'].values)


###############################################################################
def apls(input_dir, csv_file, gt_csv_file):
    # general settings
    verbose = True
    super_verbose = False
    ...
    # Compute
    value = execute(
        'gt_wkt_prop_wkt', gt_list, gp_list, root_list,
        im_loc_list,
        'length',
        'speed_m/s',
        'travel_time',
