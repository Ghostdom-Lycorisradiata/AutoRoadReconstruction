...
import time
from helper_pointcloud import write_ply
from helper_csv import pointcloud_to_csv
...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.inputs['normals'] = flat_inputs[4 * num_layers:5 * num_layers]
            self.inputs['curvatures'] = flat_inputs[5 * num_layers:6 * num_layers]
            self.inputs['features'] = flat_inputs[6 * num_layers]
            self.inputs['labels'] = flat_inputs[6 * num_layers + 1]
            self.inputs['input_inds'] = flat_inputs[6 * num_layers + 2]
            self.inputs['cloud_inds'] = flat_inputs[6 * num_layers + 3]

            self.xyz = self.inputs['xyz']
            ...
            if not exists(log_folder):
                makedirs(log_folder)
            log_file_path = join(log_folder,'log_train_' + dataset.name + '_' + str(dataset.val_split) + '.txt' )
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            f_encoder_i = self.dilated_res_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],
                                                 'Encoder_layer_' + str(i), is_training, inputs['normals'][i], inputs['curvatures'][i])
    ...
    def train(self, dataset):
        ...
        # Initialize a dictionary to store filename and corresponding apls result pairs
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                #if self.training_epoch == self.config.max_epoch:
                self.sess.run(dataset.train_init_op)
    ...
    def evaluate(self, dataset):
        ...
        for n in range(0, self.config.num_classes, 1):
            print(f"gt_classes[{n}]:", gt_classes[n])
            print(f"positive_classes[{n}]:", positive_classes[n])
            print(f"true_positive_classes[{n}]:", true_positive_classes[n])
    ...
    def get_loss(self, logits, labels, pre_cal_weights):
        ...
        # apls_result = tf.where(tf.math.is_nan(apls_result), tf.zeros_like(apls_result), apls_result)
        # final_result = tf.where(apls_result == 0, output_loss, output_loss * 0.8 + (1 - apls_result) * 0.2)
        return output_loss

    def dilated_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training, normals, curvatures):
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc = self.building_block(xyz, f_pc, neigh_idx, d_out, name + 'LFA', is_training, normals, curvatures)
    ...
    def building_block(self, xyz, feature, neigh_idx, d_out, name, is_training, normals, curvatures):
        d_in = feature.get_shape()[-1].value

        f_xyz = self.relative_pos_encoding(xyz, neigh_idx, normals, curvatures)
    ...
    def relative_pos_encoding(self, xyz, neigh_idx, normals, curvatures):
        neighbor_xyz = self.gather_neighbour(xyz, neigh_idx)
        neighbor_normals = self.gather_neighbour(normals, neigh_idx)
        neighbor_curvatures = self.gather_neighbour(curvatures, neigh_idx)
        xyz_tile = tf.tile(tf.expand_dims(xyz, axis=2), [1, 1, tf.shape(neigh_idx)[-1], 1])
        relative_xyz = xyz_tile - neighbor_xyz
        relative_dis = tf.sqrt(tf.reduce_sum(tf.square(relative_xyz), axis=-1, keepdims=True))
        # relative_feature = tf.concat([relative_dis, relative_xyz, xyz_tile, neighbor_xyz, neighbor_normals, neighbor_curvatures], axis=-1)
        relative_feature = tf.concat([relative_xyz, xyz_tile, neighbor_normals, neighbor_curvatures], axis=-1)
    ...
    def att_pooling(feature_set, d_out, name, is_training):
        ...
        f_agg = tf.reduce_sum(f_agg, axis=1) + tf.reduce_max(f_reshaped, axis=1)
