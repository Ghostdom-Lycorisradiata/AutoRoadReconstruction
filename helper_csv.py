...
from helper_pointcloud import pointcloud_to_image
from helper_pointcloud import write_image
...
def build_wkt_linestrings(G,
    ...
    # add to all_data
    for v in wkt_list:
        all_data.append(('AOI_2_Vegas_img0', v))
        # orig_fname = os.path.splitext(os.path.basename(reference_image_filename))[0]
...
def img_to_ske_G(img_refine, 
   ...
   # print a random node and edge
    ...
    if len(out_gpickle) > 0:
        ...
        # nx.write_gpickle(G, out_gpickle) 此接口 3.0 版本后已被删除
...
def preprocess(img, thresh, img_mult=255, hole_size=300,
    ...
    # if img.size < 0:
        ...
        #img = (img > (img_mult * thresh)).astype(bool)        
        img = img.astype(bool)
        remove_small_objects(img, hole_size)
        remove_small_holes(img, hole_size)
    ...
    #  effective)
        ...
        # global thresh
        #mask_thresh = (img > (img_mult * thresh))#.astype(bool)
        # print("median blur")
...
def make_skeleton(img_loc, thresh, debug, fix_borders, replicate=5,
    ...
    # print("RasterYSize:", raw_img.height)
    # print("RasterXSize:", raw_img.width)
    if raw_img.height > 1300 or raw_img.width > 1300:
        raw_img = raw_img.resize((1300, 1300))
    img_array = np.array(raw_img)
    img = img_array[:,:,skeleton_band]
...
def pointcloud_to_csv(ply_name):
    current_dir = os.path.dirname(__file__)

    # im_dir = os.path.join(current_dir, "data/sensat_tif")
    # out_dir = os.path.join(current_dir, "data/sensat_csv")
    test_path = join('test', 'after_train_ply')

    write_shps = True
    write_graphs = True
    write_csvs = False
    write_skeletons = True
    
    image_binary = pointcloud_to_image(ply_name)
    base_name = os.path.splitext(os.path.basename(ply_name))[0]
    #img_loc = write_image(image_binary, im_dir, base_name) # 返回完整路径
    img_loc = write_image(image_binary, test_path, base_name) # 返回完整路径

    if test_path is not None:
        outdirs = make_out_dirs(test_path, write_shps, write_graphs, write_csvs, write_skeletons)
    else: # just store next to the input images. 
        test_path = os.path.dirname(img_loc)
    outdirs = make_out_dirs(test_path, write_shps, write_graphs, write_csvs, write_skeletons)
    outshpdir = outdirs[0]
    outgdir = outdirs[1]
    ...
    # print("output graph: ", out_graph_filename)
    # print("output csv", outcsvsdir)
    # print("output skeleton: ", out_skeleton_filename)
    # print("image: ", img_loc)
    ...
    if ske is None: # ske is None when, preprocess() called within make_skeleton, returns an image without any road detections in the mask
        ...
        # print("first check: out_shp", out_shp)
    ...
    final_df.to_csv(os.path.join(test_path, base_name + ".csv"), index=False)  
    return base_name + ".csv"

if __name__ == "__main__":
    current_dir = os.path.dirname(__file__)
    data_dir = os.path.join(current_dir, "data/sensat_cur_ply")
    is_cambridge = 0
    if is_cambridge:
        ply_names = glob.glob(os.path.join(data_dir, '*cambridge*.ply'))
    else:
        ply_names = glob.glob(os.path.join(data_dir, '*birmingham*.ply'))

    for i in range(len(ply_names)):
        print("ply_name:", ply_names[i])
        pointcloud_to_csv(ply_names[i])
    print("finish!")